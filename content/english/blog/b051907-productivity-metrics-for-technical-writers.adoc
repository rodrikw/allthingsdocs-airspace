...
On a number of occasions I have been asked to suggest a productivity metric for technical writers, usually as part of a performance management initiative.
...
Nevertheless, it seems a little counter-intuitive for the writer to receive more productivity points for the same output.

If you want to more actively discourage writers from increasing the reviewer–writer by taking longer, you could also include the word count–writer time ratio in the formula:

.Revised formula for calculating productivity points
image::{imgpath}b051907-productivity-eqn-2.svg[]

Oras an Excel formula:

[source,excel]
----
=(((@WordCount/@WriterTime)/@BaseLineTarget)-(((@ReviewerTime/@WriterTime)-BaselineRatio)*QualityMultiplier))*PointsMultiplier
----

Fundamentally, the formula subtracts the ReviewerTime–WriterTime ratio from the WordCount–WriterTime ratio.
The effect is that changes to the reviewer time affect the score in the same ways as before.
But now, changes to the writer time are "`counteracted`" by changes to the WordCount–WriterTime ratio.

image::{imgpath}b051907-writer-reviewer-ratio-spreadsheet-4.png[]

Notice that in row 3, as the review time decreases, the productivity points resulting from the original formula (column J) and the revised formula (column P) increase.
However, when in row 4 the writer time is reduced, the resulting productivity points differ.
Using the revised formula, the productivity points increase.
I believe this is more intuitive: the writer is producing the same amount of work in less time while requiring no greater effort by the reviewers (implying constant quality).
So the writer is now more productive.
...
In a later post I intend considering other, extrinsic, ways of measuring quality (or value), such as customer ratings.
