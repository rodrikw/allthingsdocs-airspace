...
date: 2022-04-29T12:29:27+02:00
author: "Rodrik Wade"
...
Low cost is generally attractive, but I was motivated more by the desire to avoid proprietary lock-in.

* Widely used and actively supported.

Although it was not part of my initial set of requirements, I early on determined that I wanted to focus on a Jamstack solution rather than one employing a content management system such as WordPress.
...
=== Markup
After six years of using a conventional help authoring tool, I wanted to try authoring using a light-weight markup language.
I investigated reStructuredText (RST) and Markdown before stumbling across AsciiDoc.
...
By comparison with RST, AsciiDoc is generally easier to remember.
Compare:
...
If you choose AsciiDoc as your markup, that affects your choice of editor and static site generator.
...
* https://plugins.jetbrains.com/plugin/12175-grazie/[Grazie^].
A spelling, grammar, and style checker.
...
For example, I have created a live template to quickly insert the date and time in ISO format: 2022-04-27T11:06:40+02:00
...
* Code folding, which enables AsciiDoc sections and block to be collapsed or expanded.
...
However, I don't recommend it if you are intending to use Antora as the static site generator.

=== Version control system

Git is the dominant version control system (VCS) for open-source projects, and is becoming widespread in other environments.
...
NOTE: Antora expects content to be stored in one or more Git repos.

==== Git host

GitHub and GitLab are the two most widely used cloud-based Git hosts.
For my purposes, there didn't appear to be much to distinguish the two. I've found GitHub easy to use, and it offers many useful features such as https://docs.github.com/en/actions/using-workflows/about-workflows[workflows^].
...
If you do decide to use GitHub, you'll probably want to install GitHub Desktop.
It makes tasks such as pushing changes to GitHub or branching a repo much easier than doing so using the command line.

I also use the Git functionality provided by IntelliJ IDEA a lot.
...
* It does not yet support automatic generation of PDFs, but development of this functionality is well advanced (as of 28 April 2022).
...
I'm glad that I did as I've been impressed with the ease of use and functionality it offers, not to mention that it has a free https://www.netlify.com/pricing/[Starter tier^].
...
Unfortunately, it's not particularly cheap.
...
It's powerful (certainly exceeding anything I might require), although it's not always intuitive.
